...
결국 IService의 핵심 매서드인 runSomething을 쓰고 싶은 것이고, 그것은 IService를 구현한 Service에 있어. 그런데 Proxy를 앞세워서 Proxy에서 실행해. 이 Proxy는 Service를 불러오고 ㅇㅇ <br>

근데 프록시 패턴은 중간에 다른 로직을 수행하더라도, 받아온 값 그대로 반환하고 데코레이터 패턴은 뭔가 더해서 반환함
