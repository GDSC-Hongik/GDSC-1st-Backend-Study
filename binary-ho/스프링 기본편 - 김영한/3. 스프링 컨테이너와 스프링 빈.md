...
```
**`ApplicationContext`를 스프링 컨테이너라고 부른다.** 옛날에는 들어도 들어도 도무지 와닿지가 않았는데, OS도 배우고, CS 전반에 대한 지식이 늘어가며 Context의 의미가 와닿기 시작한다. <br>
Context는 직역하면 '문맥'인데, 어떤 **프로그램 흐름이 기록되며 이루어지는 공간이라는 느낌으로 이해하면 좋다.** <br>
프로그램의 동작하며 모든 것이 기록되는데 그런 것들이 적히는 공간이다. 이런 맥락에서 `컨테이너` 라는 표현은 적절하다. <br>
`AnnotationConfigApplicationContext`는 ApplicationContext 인터페이스의 구현체이다. <br> <br>
...
- 단순히 자바 코드를 호출하는 것을 넘어, 싱글톤으로 관리되는 등 스프링에게 관리를 넘김
