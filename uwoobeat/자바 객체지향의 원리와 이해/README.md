...
- 잘 이해가 가지 않는다. 일단 자바 가상 머신 (JVM)에 대해 알아보자.
  ...
  - 어떻게?
    - 자바 컴파일러는 자바 소스 파일(_.java)을 컴파일하여 자바 목적 파일, 즉 자바 바이트코드(_.class)로 변환한다.
...
- 의존성이란?
  - 다음과 같은 상황을 고려하자.
    - 사람이 자동차를 만든다
    - 자동차는 내부에서 바퀴를 생산한다.
  - 차와 바퀴의 관계에서 생성자 `Car()`과 그 안의 `new Tire();` 에서, 의존성은 new라고 할 수 있다.
- 다음과 같은 의사 코드를 고려하자.
  - 자동차는 내부에서 생성되는 바퀴에 의존한다.
  - 운전자는 자동차를 사용한다.
  - 운전자가 자동차에 의존한다.
  - 자동차의 생성자 코드에서 tire 속성에 새로운 타이어를 생성하여 참조하도록 하였다.
- 의존성 주입을 통하여 이러한 의존성을 해결해야 한다.
  - 주입이란 “외부에서” 라는 말을 내포한다.
  - 자동차 내부에서 타이어를 만드는 것이 아니라, 외부에서 만들어진 타이어를 자동차에 장착하는 것이 의존성 주입이다.
...
// 운전자가 타이어를 생산한다.
// 운전자가 자동차를 생산하면서 생산된 타이어를 장착한다.
```

- 앞에서는 Car 객체가 직접 Tire를 생산 (Tire 생성자 호출)했다.
  - 이는 유연성이 떨어진다.
  - Car 객체가 직접 어떤 타이어를 생산할 지 결정해야 하기 때문이다.
  - 이러한 결정은 외부(운전자)에서 내리고, Car는 자동차의 구현에만 집중하도록 하는 것이 좋다.
- 여기서는 외부에서 Tire를 생산한 뒤, Car의 생성자 파라미터로 Tire을 참조하게 했다.
  - 이러한 의존성 주입을 사용할 경우, Car는 Tire 인터페이스를 구현한 객체를 파라미터로 넘겨받기만 하면 정상 작동한다.
  - KoreaTire 말고도 ChinaTire, USTire 등 어떤 구현체가 들어오더라도 대응할 수 있기에, 확장성이 좋다. (확장에 열려있다)
  - 또한 Tire 구현체 코드가 변화하더라도 Car 코드는 변화하지 않으므로 컴파일과 재배포에 유리하다 (변화에 닫혀있다)
  - 즉, 개방 폐쇄 원칙이 적용된 것
    - 현실 세계에서는 인터페이스보다는 표준화라는 비유가 더 잘 와닿는다. 참고
- 이는 전략 패턴에 해당되기도 한다. (?)
...
- 생성자를 사용하면 생산할 때만 타이어를 장착할 수 있다는 단점이 있다.
- 속성을 사용하면 setter 메서드 호출을 통해 원할 때 타이어를 장착할 수 있다.
  - 이렇게 보면 속성 방식이 좋아보이지만, 한번 주입된 의존성은 잘 수정되지 않으므로 생성자를 더 선호하는 편이다.

### 스프링을 통한 의존성 주입 - XML 파일

```java
ApplicationContext context = new ClassPathXmlApplicationContext(
	"expert002.xml", Driver.class
);
...
// 종합 쇼핑몰 (스프링 프레임워크)에서 타이어를 구매한다
// 종합 쇼핑몰에서 자동차를 구매한다
// 자동차에 타이어를 장착한다
```

- 스프링은 생성자와 속성을 통한 의존성 주입 모두 지원한다.
  - 여기서는 속성만 사용했다.
- context는 프레임워크에 대한 정보를 받아와서, getBean으로 필요한 객체들을 참조하도록 한다.
  - 쇼핑몰의 물품은 어떻게 등록되어 있을까 (스프링의 빈은 어디 있을까?)
    - Spring Bean Configuration에서 가능
    - <bean> 태그를 통한 등록
- 자동차 브랜드 등이 바뀌더라도, 재컴파일/배포 없이 XML 파일만 수정하면 된다.

### XML에서 속성 주입

```java
ApplicationContext context = new ClassPathXmlApplicationContext("expert003.xml");
Car car = context.getBeat("car", Car.class);
...
- `Tire tire = ...` 과 `car.setTire(tire)` 이 없어진 것을 확인할 수 있다.
- 이것을 XML의 `<property name="tire" ref="koreaTire"></property>` 로 대체한 것이다.
  - Car 생성자 호출과 클래스의 setter 메서드를 <property> 태그로 대체했다.
...
// 의사 코드는 동일하다.
// XML에서 <property> 태그가 사라진 것을 볼 수 있다.
```

- import 문을 추가하고 어노테이션을 추가하면 스프링이 설정 파일로 setter 대신 속성을 주입한다.
- @Autowired의 의미는 뭘까? 스프링이 설정 파일을 보고 자동으로 해당 속성의 setter 역할을 수행하주겠다는 것이다.
  - 해당 속성의 이름이 설정 파일에 존재하는 bean의 ID와 일치한다면 ID의 클래스 생성자를 실행하여 맵핑시켜준 것 같다. (추측)
- 만약 bean의 id 속성이 없어진다면? 그래도 정상 작동한다. 왜일까?
  - @Autowired는 타입 기준으로 매칭해주기 때문이다. id는 그 다음!
    - @Autowired는 속성의 타입을 구현한 빈이 있는지 확인한다.
    - 이후 빈이 하나만 있는지 확인한다. 있다면 빈을 객체에 할당한다.
      - 만약 유니크한 빈이 아니라면 에러가 발생하니, 항상 빈의 id를 잘 작성하자.
...
- @Autowired 대신 @Resource를 사용한다.
  - 자동으로 묶어준다… 보다는 해당 클래스에 필요한 자원으로 보는 것이 적절
- 차이는?
  - Autowired는 스프링의 어노테이션, Resource는 자바 표준 어노테이션이다.
    - 스프링을 안쓴다면 후자
  - Resource는 id → type 순 매칭이다.
